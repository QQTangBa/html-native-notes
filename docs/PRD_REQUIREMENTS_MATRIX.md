...
| A2 | MCP/CLI/HTTP/file-watch generated HTML enters Vault within 10 seconds with source, summary, thumbnail | `npm run test:bridge` and desktop bridge flow | Partial | HTTP-to-manifest intake exists; TS thumbnail generator can create a PNG and update Vault Library status to ready; MCP, desktop UI, summary quality, and 10-second desktop flow still pending |
