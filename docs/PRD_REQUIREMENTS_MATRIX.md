...
| --- | --- | --- | --- | --- | --- |
| F1 | Vault home with folder-like library, cards/list, thumbnails, title, tags, updated time, source, search, filters | React `features/vault`, Rust `core/vault` | component tests, desktop flow, 100-fixture search test | Minimal TS Vault Library data contract, API boundary, React VaultHome component, and App loading path started; Tauri command wiring, real thumbnail generation, Rust Vault core, and 100-fixture desktop flow missing | `npm run test -- tests/integration/bridge/vaultLibrary.test.ts` PASS for AI-generated HTML entering manifest and searchable/filterable metadata; `npm run test -- tests/integration/api.test.ts` PASS for `/api/vault/library`; `npm run test -- tests/unit/VaultHome.test.tsx` PASS for dense desktop Vault UI rendering; `npm run test -- tests/unit/App.test.tsx` PASS for loading Vault assets into the App shell |
