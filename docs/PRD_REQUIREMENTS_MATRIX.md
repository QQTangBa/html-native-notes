...
| F2 | Agent Bridge through MCP, CLI, HTTP API, offline inbox, file watching | `bridge/*`, Rust inbox/watcher commands | MCP integration, HTTP integration, CLI integration, inbox startup, watcher test | Protocol schema started; transports missing | `npm run test -- tests/unit/agentBridgeProtocol.test.ts` PASS for shared request schemas and normalization |
