...
| --- | --- | --- | --- | --- | --- |
| F1 | Vault home with folder-like library, cards/list, thumbnails, title, tags, updated time, source, search, filters | React `features/vault`, Rust `core/vault` | component tests, desktop flow, 100-fixture search test | Minimal TS Vault Library data contract, API boundary, React VaultHome component, App loading path, browser runtime checks, Playwright thumbnail generation, and toolbar trigger started; Tauri command wiring, Rust Vault core, background thumbnail queue, and 100-fixture desktop flow missing | `npm run test -- tests/integration/bridge/vaultLibrary.test.ts` PASS for AI-generated HTML entering manifest and searchable/filterable metadata; `npm run test -- tests/integration/bridge/thumbnailGenerator.test.ts` PASS for real PNG thumbnail generation and source-hash preservation; `npm run test -- tests/integration/api.test.ts` PASS for `/api/vault/library` and `POST /api/vault/thumbnails/generate`; `npm run test -- tests/unit/VaultHome.test.tsx` PASS for dense desktop Vault UI rendering and thumbnail action states; `npm run test -- tests/unit/App.test.tsx` PASS for loading Vault assets and refreshing thumbnails from the App shell; Playwright browser checks saved `test-results/desktop-launch/vault-home-runtime-html.png`, `test-results/desktop-launch/vault-thumbnail-action-pending.png`, and `test-results/desktop-launch/vault-thumbnail-action-ready.png` |
...
| --- | --- | --- | --- | --- |
| A1 | Import 100 mixed HTML/Markdown files; 95% open/search/preview | `npm run test:fixtures:mixed100` plus desktop import screenshot | Partial | Single AI-generated HTML fixture can enter Vault Library and be searched/filtered; 100 mixed fixture, open/preview flow, and desktop screenshot pending |
