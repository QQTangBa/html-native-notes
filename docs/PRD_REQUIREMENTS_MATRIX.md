...
| --- | --- | --- | --- | --- | --- |
| F1 | Vault home with folder-like library, cards/list, thumbnails, title, tags, updated time, source, search, filters | React `features/vault`, Rust `core/vault` | component tests, desktop flow, 100-fixture search test | Minimal TS Vault Library data contract, API boundary, React VaultHome component, App loading path, browser runtime check, and Playwright thumbnail generation started; Tauri command wiring, Rust Vault core, background thumbnail queue, and 100-fixture desktop flow missing | `npm run test -- tests/integration/bridge/vaultLibrary.test.ts` PASS for AI-generated HTML entering manifest and searchable/filterable metadata; `npm run test -- tests/integration/bridge/thumbnailGenerator.test.ts` PASS for real PNG thumbnail generation and source-hash preservation; `npm run test -- tests/integration/api.test.ts` PASS for `/api/vault/library` and `POST /api/vault/thumbnails/generate`; `npm run test -- tests/unit/VaultHome.test.tsx` PASS for dense desktop Vault UI rendering; `npm run test -- tests/unit/App.test.tsx` PASS for loading Vault assets into the App shell; Playwright browser check saved `test-results/desktop-launch/vault-home-runtime-html.png` |
...
| A2 | MCP/CLI/HTTP/file-watch generated HTML enters Vault within 10 seconds with source, summary, thumbnail | `npm run test:bridge` and desktop bridge flow | Partial | HTTP-to-manifest intake exists; CLI/API can trigger TS thumbnail generation and update Vault Library status to ready; MCP, desktop UI, summary quality, and 10-second desktop flow still pending |
