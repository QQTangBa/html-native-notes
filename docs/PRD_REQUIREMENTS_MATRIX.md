...
| --- | --- | --- | --- | --- | --- |
| F1 | Vault home with folder-like library, cards/list, thumbnails, title, tags, updated time, source, search, filters | React `features/vault`, Rust `core/vault` | component tests, desktop flow, 100-fixture search test | Minimal TS Vault Library data contract and React VaultHome component started; App/Tauri data wiring, real thumbnail generation, Rust Vault core, and 100-fixture flow missing | `npm run test -- tests/integration/bridge/vaultLibrary.test.ts` PASS for AI-generated HTML entering manifest and searchable/filterable metadata; `npm run test -- tests/unit/VaultHome.test.tsx` PASS for dense desktop Vault UI rendering, card/list toggle, search, tag/source/folder filters, and thumbnail status |
