...
| --- | --- | --- | --- | --- | --- |
| F1 | Vault home with folder-like library, cards/list, thumbnails, title, tags, updated time, source, search, filters | React `features/vault`, Rust `core/vault` | component tests, desktop flow, 100-fixture search test | Minimal TS Vault Library data contract started; desktop UI, real thumbnail generation, Rust Vault core, and 100-fixture flow missing | `npm run test -- tests/integration/bridge/vaultLibrary.test.ts` PASS for AI-generated HTML entering manifest, card/list metadata, folder summaries, title/tags/summary/source enrichment, thumbnail pending path, search, tag filter, source-agent filter, kind filter, and folder filter |
